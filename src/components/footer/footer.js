...
export default function Footer() {
  ...
  <p>{info?.alamat || "Bogor, Jawa Barat"}</p>
</div>

              <div className="flex items-start gap-3 my-4">
  <Mail className="mt-0.5 shrink-0 text-amber-500" size={18} />
  <p>{info?.email || "wahyurifai63@gmail.com"}</p>
</div>
              ...
              {info?.whatsapp && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 my-4 text-white transition hover:bg-green-700"
                >
                  <FaWhatsapp size={18} />
                  Chat WhatsApp
                </a>
